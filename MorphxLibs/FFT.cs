...
namespace MorphxLibs {
    public static class FFT {
        ...
        public static void FourierTransform(int fftSize,
                                        double[] waveInL, ref Complex[] fftOutL,
                                        bool doInverse) {
            int numBits;
            int k;
            ...
            Complex initialAngle = new Complex(1);
            Complex angle;
            ...
            if(lastFFTSize != fftSize) {
                ...
                for(int i = 0; i < fftSize; i++) {
                    ...
                    fftOutL[rBits[i]] = new Complex(waveInL[i]);
                }
            } else {
                for(int i = 0; i < fftSize; i++) {
                    fftOutL[rBits[i]] = new Complex(waveInL[i]);
            ...
            do {
                ...
                for(int i = 0; i < fftSize; i += blockSize) {
                    ...
                    for(int j = i; j < blockEnd + i; j++) {
                        ...
                        angle -= new Complex(alpha * angle.R + beta * angle.I,
        ...
        public static void FourierTransform(int fftSize,
                                        double[] waveInL, ref Complex[] fftOutL,
                                        double[] waveInR, ref Complex[] fftOutR,
                                        bool doInverse) {
            int numBits;
            int k;
            ...
            if(lastFFTSize != fftSize) {
                ...
                for(int i = 0; i < fftSize; i++) {
                    ...
                    fftOutL[rBits[i]] = new Complex(waveInL[i]);
                    fftOutR[rBits[i]] = new Complex(waveInR[i]);
                }
            } else {
                for(int i = 0; i < fftSize; i++) {
                    fftOutL[rBits[i]] = new Complex(waveInL[i]);
